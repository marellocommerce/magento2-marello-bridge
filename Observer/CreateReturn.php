        ...
        if ($object instanceof Rma) {
            try {
                $result = $this->entityQueueRepository->findOneByIdAndEventType(
                    $object->getId(),
                    QueueEventTypeInterface::QUEUE_EVENT_TYPE_RMA_CREATE
                );
                ...
                $queueEnitity->setMagId($object->getId());
                $queueEnitity->setEventType(QueueEventTypeInterface::QUEUE_EVENT_TYPE_RMA_CREATE);
                $queueEnitity->setEntityData(['entityAlias' => 'rma', 'entityClass' => get_class($object)]);
